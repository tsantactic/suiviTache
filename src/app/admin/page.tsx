...
function UserRow({ u, onRole, onEmail, onPassword, onDelete }: { u: any; onRole: any; onEmail: any; onPassword: any; onDelete: any }) {
  ...
  return (
    <tr className="dark:text-slate-200">
      <td className="px-2 py-2 text-xs">{u.email}<br/><span className={`px-1.5 py-0.5 rounded-full text-[10px] ${u.role==='admin'?'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-200':'bg-slate-100 dark:bg-slate-700 dark:text-slate-200'}`}>{u.role}</span></td>
      <td className="px-2 py-2"><select value={u.role} onChange={(e) => onRole(u.id, e.target.value)} className="border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white rounded px-1 py-1 text-xs"><option value="user">user</option><option value="admin">admin</option></select></td>
      <td className="px-2 py-2"><div className="flex gap-1"><input value={email} onChange={(e)=>setEmail(e.target.value)} className="border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white rounded px-1 py-1 text-xs w-28" /><button onClick={()=>onEmail(u.id, email)} className="px-2 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700">OK</button></div></td>
      <td className="px-2 py-2"><div className="flex gap-1"><input type="password" placeholder="••••" value={pwd} onChange={(e)=>setPwd(e.target.value)} className="border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white placeholder:text-slate-400 rounded px-1 py-1 text-xs w-24" /><button onClick={()=>{onPassword(u.id,pwd); setPwd("");}} className="px-2 py-1 bg-slate-900 dark:bg-slate-700 text-white rounded text-xs hover:bg-slate-800">OK</button></div></td>
  ...
  if (!isAdmin) return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="max-w-3xl mx-auto p-8">
        <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 p-6 rounded-xl">
          <h1 className="font-bold text-amber-900 dark:text-amber-200">Accès réservé aux admins</h1>
          <p className="text-sm text-amber-800 dark:text-amber-300 mt-1">Votre compte n&apos;a pas le rôle admin. Pour vous promouvoir, exécutez dans Supabase SQL :</p>
          {/* eslint-disable-next-line react/no-unescaped-entities */}
          <code className="block mt-3 bg-slate-100 dark:bg-slate-800 dark:text-slate-200 p-3 rounded border border-slate-200 dark:border-slate-700 text-xs overflow-auto">update profiles set role=&apos;admin&apos; where email=&apos;VOTRE_EMAIL&apos;;</code>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {message && <div className="mt-4 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-200 text-sm p-3 rounded-lg">{message}</div>}

        <div className="mt-6 grid gap-4">
          <div className="bg-slate-100 dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-semibold dark:text-white">Inviter une personne par email</h3>
            ...
            <form onSubmit={inviteUser} className="mt-3 flex flex-col sm:flex-row gap-2">
              <input type="email" required value={inviteEmail} onChange={(e)=>setInviteEmail(e.target.value)} placeholder="invite@exemple.com" className="flex-1 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white placeholder:text-slate-400 rounded-lg px-3 py-2 text-sm" />
              <select value={inviteRole} onChange={(e)=>setInviteRole(e.target.value)} className="border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white rounded-lg px-3 py-2 text-sm"><option value="user">user</option><option value="admin">admin</option></select>
          ...
          <div className="bg-slate-100 dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-semibold dark:text-white">Exporter les données</h3>
            ...
            <div className="mt-4 flex gap-3">
              <button onClick={handleExport} className="bg-slate-900 dark:bg-slate-700 text-white px-4 py-2 rounded-lg hover:bg-slate-800">Exporter JSON</button>
              <button onClick={handleExportCSV} className="px-4 py-2 border border-slate-300 dark:border-slate-600 dark:text-white rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700">Exporter CSV (tâches)</button>
            </div>
          </div>

          <div className="bg-slate-100 dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-semibold dark:text-white">Importer les données</h3>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">Format attendu : &#123; projects: [], tasks: [], exported_at &#125; — généré par l&apos;export ci-dessus.</p>
          </div>

          <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 p-4 rounded-xl">
            <h4 className="font-medium text-purple-900 dark:text-purple-200 text-sm">Astuce mise à jour future</h4>
            <p className="text-xs text-purple-800 dark:text-purple-300 mt-1">Avant une grosse mise à jour, exportez → déployez → ré-importez si besoin. Vous pouvez aussi utiliser ce JSON pour migrer vers un autre Supabase.</p>
          </div>
        </div>
      </div>
    </div>
